...
BaseConnector::handle_connect (const TRB_Asynch_Connect::Result &result)
{
    ...
    if (error != 0 || handle == ACE_INVALID_HANDLE)
    {
        ...
        TRB_Sock_Addr::to_string (addr, buf0, sizeof(buf0),0);
        TRB_Sock_Addr::to_string (addr, buf1, sizeof(buf1),1);
        ...
        // ACE_DEBUG ((LM_DEBUG,
        //             ACE_TEXT ("(%t) BaseConnector: %s(%s) act=%u error=%d %p\n"),
        //             buf0,
        //             buf1,
        //             (unsigned int) result.act (),
        //             error,
