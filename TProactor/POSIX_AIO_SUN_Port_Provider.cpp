...
int 
TRB_POSIX_AIO_SUN_Port_Provider::get_specific_flags () const
{
    //return 0;  

    return (TRB_POSIX_AIO_FD_Info::KEEP_READ_READY |
            TRB_POSIX_AIO_FD_Info::KEEP_WRITE_READY );
}
                                                   ...
                                                   int remain_mask)
{
...

    {
        ...
        FD_Guard fd_guard (get_repository(), fd, false);

        //refresh interest
        interest = info->get_interest ();
        ...
        rc = ::port_associate (this->fd_poller_, PORT_SOURCE_FD, fd, events, 0);
    }
